...
//  4. Persistence is batched through scope.mutate(): v2 wrote to settings.yaml on
//     every `input` event, i.e. dozens of disk writes per slider drag.
		...
		function apply(ctx) {
			...
			const render = () => {
				try {
					...
					const glassOn = view !== null && view.enabled === true && view.glass === true;
